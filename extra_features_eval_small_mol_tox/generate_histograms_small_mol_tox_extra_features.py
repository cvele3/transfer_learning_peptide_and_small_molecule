...
PROCESSED_FILE = "../inflated_models/large_layers_overtrained_small_mol_tox_pred.pkl"
...
def extract_fold_number(filename):
    ...
    # Adjust logic if filename format differs slightly, but this works for standard naming.
    try:
    ...
    for path in model_paths:
        try:
            model = load_model(
                ...
                compile=False # Optimization: don't compile if just predicting
            )
            models.append((os.path.basename(path), model))
        except Exception as e:
            print(f"Failed to load model {path}: {e}")
            
    # Sort models by fold number
    models = sorted(models, key=lambda x: extract_fold_number(x[0]))
    return models

def generate_histogram(model_name, y_true_all, y_pred_prob_all):
    """
    Generates and saves a histogram for a given model's aggregated predictions.
    Requirements:
    - X-Axis: Predicted Probability (0.0 to 1.0)
    - Bins: Width 0.05
    - Colors: Blue (Correct), Red (Incorrect)
    - Transparency: alpha=0.5
    - Labels: Count on top of bars
    - Formatting: Remove top/right spines, horizontal grid, axis labels.
    """
    ...
    # Plot histograms
    # We use plt.hist which returns counts (n), bins, and patches
    ...
    # We need to handle potential overlaps. Since bars overlap (alpha=0.5), we can just label them.
    # To avoid text collision if bars are same height, we might need offset, but simple implementation first.
    ...
    def add_labels(counts, bins, color, vertical_offset=5):
        for count, x in zip(counts, bins):
            if count > 0:
                plt.text(
                    x + 0.025, # Center of bin (bin width is 0.05)
    ...
    # Add labels for correct (Blue) and incorrect (Red)
    # To avoid overlap, we check which bar is taller at each bin
    # and adjust the vertical position of the label for the shorter bar if needed.
    # However, since the bars are overlapping (alpha=0.5), the visual overlap of bars is handled by matplotlib.
    # The labels are placed on top of their respective bars.
    # If the counts are similar, the labels might overlap.
    # To fix this, we can offset the labels based on their relative positions.

    for i in range(len(bins_correct) - 1): # bins_correct has one more element than n_correct
        ...
        # If counts are close, shift one label up or down, or horizontally?
        # Vertical shift is safer.
        
        # Convert counts to display coordinates logic (simplified)
        # If the top of one bar is close to the top of the other...
        # Actually, simpler logic:
        # If Blue (Correct) is taller, Blue label is higher. Red label is lower.
        # If Red is taller, Red label is higher.
        # If they are close, the lower one might be inside the taller bar or just below the top one.
        # But we want to see both.
        
        # Let's adjust the vertical_offset dynamically.
        # If counts are within a certain threshold (e.g., 5% of max y or just distinct count distance)
        # For simplicity, if abs(count_correct - count_incorrect) < threshold, we shift the higher one up more.
        
        # A better approach for readable labels on overlapping histograms:
        # Place label for the TALLER bar slightly higher.
        # Place label for the SHORTER bar slightly lower than its top (inside) OR ensure enough vertical separation.
        # Given the user wants "on top of each bar", strictly speaking:
        
        # Let's check vertical distance.
        vertical_dist = abs(count_correct - count_incorrect)
        min_dist = 40 # Minimum vertical distance to avoid text overlap (approximate pixels/units)
        
        pos_correct = count_correct + offset_correct
        pos_incorrect = count_incorrect + offset_incorrect
        
        if vertical_dist < min_dist:
             # Overlap likely. Shift the label of the TALLER bar UP further.
             if count_correct >= count_incorrect:
                 pos_correct += 25 # Shift Blue up
             else:
                 pos_incorrect += 25 # Shift Red up
...
# Paths defined based on user request and previous context
baseline_models = load_models_by_pattern("../extra_features_baseline_small_mol_tox/baseline_small_mol_tox_fold_*.h5")
method1_models = load_models_by_pattern("../transfer_learning_extra_features_p_to_smt/freeze_gnn_peptide_to_smile_mol_tox_fold_*.h5")
method2_models = load_models_by_pattern("../transfer_learning_extra_features_p_to_smt/freeze_readout_peptide_to_smile_mol_tox_fold_*.h5")
method3_models = load_models_by_pattern("../transfer_learning_extra_features_p_to_smt/freeze_all_peptide_to_smile_mol_tox_fold_*.h5")
method4_models = load_models_by_pattern("../transfer_learning_extra_features_p_to_smt/gradual_unfreezing_peptide_to_smile_mol_tox_fold_*.h5")
