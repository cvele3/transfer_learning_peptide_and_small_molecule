"""
Script to evaluate all SMT (Small Molecule Toxicity) models across three GNN layer configurations,
populate Excel table with results, and highlight best values.
"""
...
def evaluate_configuration(config_name, processed_file, splits_file, model_patterns):
    ...
    Returns:
        metrics_summary: Dict with mean/std for each model group and metric
    ...
    for group in model_groups:
        ...
        for metric in metric_names:
            ...
            if len(values) > 0:
                ...
                metrics_summary[group][metric] = {"mean": np.nan, "std": np.nan}
...
def main():
    ...
    config1 = {
        ...
        "model_patterns": {
            "baseline": "extra_features_baseline_small_mol_tox/baseline_small_mol_tox_fold_*.h5",
            "method1": "transfer_learning_extra_features_p_to_smt/freeze_gnn_peptide_to_smile_mol_tox_fold_*.h5",
            "method2": "transfer_learning_extra_features_p_to_smt/freeze_readout_peptide_to_smile_mol_tox_fold_*.h5",
            "method3": "transfer_learning_extra_features_p_to_smt/freeze_all_peptide_to_smile_mol_tox_fold_*.h5",
            "method4": "transfer_learning_extra_features_p_to_smt/gradual_unfreezing_peptide_to_smile_mol_tox_fold_*.h5",
        }
    }
    ...
    config2 = {
        ...
        "splits_file": "large_layers_cv_splits/large_layers_cv_splits_small_mol_tox_pred.pkl",
        "model_patterns": {
            "baseline": "large_layers_baseline_small_mol_tox/large_layers_baseline_small_mol_tox_fold_*.h5",
            "method1": "large_layers_transfer_learning_p_to_smt/LL_freeze_gnn_peptide_to_smile_mol_tox_fold_*.h5",
            "method2": "large_layers_transfer_learning_p_to_smt/LL_freeze_readout_peptide_to_smile_mol_tox_fold_*.h5",
            "method3": "large_layers_transfer_learning_p_to_smt/LL_freeze_all_peptide_to_smile_mol_tox_fold_*.h5",
            "method4": "large_layers_transfer_learning_p_to_smt/LL_gradual_unfreezing_peptide_to_smile_mol_tox_fold_*.h5",
        }
    }
    ...
    config3 = {
        ...
        "splits_file": "large_layers_cv_splits/large_layers_cv_splits_small_mol_tox_pred.pkl",
        "model_patterns": {
            "baseline": "inflated_baseline_small_mol_tox/inflated_baseline_small_mol_tox_fold*.h5",
            "method1": "inflated_transfer_learning_p_to_smt/inflated_freeze_gnn_peptide_to_smile_mol_tox_fold_*.h5",
            "method2": "inflated_transfer_learning_p_to_smt/inflated_freeze_readout_peptide_to_smile_mol_tox_fold_*.h5",
            "method3": "inflated_transfer_learning_p_to_smt/inflated_freeze_all_peptide_to_smile_mol_tox_fold_*.h5",
            "method4": "inflated_transfer_learning_p_to_smt/inflated_gradual_unfreezing_peptide_to_smile_mol_tox_fold_*.h5",
        }
    }
    ...
    for config in configurations:
        ...
        # Find best values for each metric in this configuration
        best_values = {}
        best_models = {}
        for metric in metric_names:
            best_val = -np.inf
            best_model = None
            for group in model_groups:
                val = metrics_summary[group][metric]["mean"]
        ...
        for i, group in enumerate(model_groups):
            ...
            for col, metric in enumerate(metric_names, start=3):
                ...
                if is_best and not np.isnan(mean_val):
                    ...
                    if best_significant_vs_baseline.get(metric, False):
                        cell.value = f"{mean_val:.4f} ± {std_val:.4f} *"
                        cell.font = Font(bold=True)
                    else:
                        cell.font = Font(bold=True)
    ...
    # Save workbook
    output_file = "eval-book-smt-results.xlsx"
    wb.save(output_file)
