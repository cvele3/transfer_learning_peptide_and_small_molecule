...
Model:
  - SVM s RBF jezgrom
  - C = 5, gamma = 0.001, class_weight = 1
...
SVM_C = 5
SVM_GAMMA = 0.001
DECISION_THRESHOLD = -0.8  # prilagodjeni prag na decision_function izlazu
...
def main():
    ...
    for fold_idx, split in enumerate(cv_splits, 1):
        ...
        # StandardScaler - fit na train, transform na train i test
        ...
        svm = SVC(
            ...
            C=SVM_C,
            gamma=SVM_GAMMA,
            class_weight="balanced",  # kompenzira nebalansiran dataset (33%/67%)
            probability=True,
        ...
        print(f"  Fold {fold_idx:2d}:", end="")
        for label, key in metric_order:
            print(f" {label}={metrics[key]:.4f}", end="")
    ...
    metric_names = [key for _, key in metric_order]
    ...
    for metric_name in metric_names:
        ...
        display_name = next(label for label, key in metric_order if key == metric_name)
        print(f"  {display_name:15s}:  {mean_val:.4f} +/- {std_val:.4f}")
    ...
    for metric_name in metric_names:
        values = [m[metric_name] for m in fold_metrics]
        label = next(lbl for lbl, key in metric_order if key == metric_name)
        mean_row[label] = np.mean(values)
        std_row[label] = np.std(values)
    ...
    # ------------------------------------------------------------------
    fold_results_file = os.path.join(SCRIPT_DIR, "dpc_svm_fold_results.pkl")
    ...
    print("\n[DONE] DPC + SVM evaluacija zavrsena.")
