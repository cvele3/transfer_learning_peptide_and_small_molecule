...
VERZIJA BEZ STANDARDSCALER-A (kao SVMlight).
...
Model:
  ...
  - BEZ StandardScaler-a (kao SVMlight - ocekuje da su podaci vec skalirani)
...
Napomene:
  ...
  - BEZ StandardScaler-a: DPC značajke su već normalizirane (frequency 0-1),
    što je dovoljno za SVMlight pristup (kao u originalnom radu)
"""
...
# Napomena: Koristimo svm.predict() koji koristi defaultni threshold 0.0
# (ne koristimo custom threshold -0.4 u ovoj verziji)
SVM_THRESHOLD = 0.0  # Default threshold za svm.predict()
...
def main():
    ...
    print("  VERZIJA BEZ STANDARDSCALER-A (kao SVMlight)")
    print("=" * 70)
    print(f"\n  Parametri:")
    ...
    print(f"    Feature scaling:    NEMA (kao SVMlight - DPC je vec normaliziran)")
    print(f"    Prediction method: svm.predict() (default threshold 0.0)")
    ...
    print("  (DPC je vec normaliziran: frequency 0-1, nema potrebe za StandardScaler)")
    ...
    print(f"  BEZ StandardScaler-a (kao SVMlight pristup)")
    print(f"{'-' * 70}")

    fold_metrics = []
    models_dir = os.path.join(SCRIPT_DIR, "models_no_scaler")
    ...
    for fold_idx, split in enumerate(cv_splits, 1):
        ...
        # BEZ StandardScaler-a - koristimo direktno DPC značajke
        # (DPC je već normaliziran kao frequency 0-1, što je dovoljno za SVMlight pristup)
        # Napomena: Ovo je bliže originalnom SVMlight pristupu iz rada
        ...
        svm.fit(X_train_full, y_train_full)  # Direktno bez skaliranja

        # Predikcija na test skupu
        y_pred = svm.predict(X_test)  # Direktno bez skaliranja
        y_pred_proba = svm.predict_proba(X_test)[:, 1]
        
        # Dijagnostika: provjeri distribuciju predikcija
        unique_pred, counts_pred = np.unique(y_pred, return_counts=True)
        pred_dist = dict(zip(unique_pred, counts_pred))
        
        # Provjeri distribuciju vjerojatnosti
        proba_mean = np.mean(y_pred_proba)
        proba_std = np.std(y_pred_proba)
        proba_min = np.min(y_pred_proba)
        proba_max = np.max(y_pred_proba)

        # Metrike
        metrics = compute_metrics(y_test, y_pred, y_pred_proba)
        
        # Dodaj dijagnostičke informacije
        ...
        # Spremi model (bez scalera)
        model_path = os.path.join(models_dir, f"dpc_svm_no_scaler_fold_{fold_idx}.pkl")
        with open(model_path, "wb") as f:
            pickle.dump({"model": svm}, f)  # Samo model, bez scalera

        # Dijagnostika
        diag = metrics.get("_diagnostics", {})
        pred_dist = diag.get("pred_distribution", {})
        proba_info = f"proba: {diag.get('proba_mean', 0):.3f}±{diag.get('proba_std', 0):.3f}"
        ...
        print(f"           Pred: {pred_dist}  {proba_info}  "
              f"SV: {diag.get('n_support_vectors', 0)}  "
              f"-> Saved {model_path}")
    ...
    print("  (BEZ StandardScaler-a - kao SVMlight pristup)")
    print(f"{'=' * 70}")
    ...
    # ------------------------------------------------------------------
    results_file = os.path.join(SCRIPT_DIR, "DPC_SVM_results_no_scaler.xlsx")
    ...
    # ------------------------------------------------------------------
    fold_results_file = os.path.join(SCRIPT_DIR, "dpc_svm_fold_results_no_scaler.pkl")
    ...
    fold_results_data = {
        ...
        "svm_params": {
            ...
            "scaler": "NONE (kao SVMlight - DPC je vec normaliziran)",
            "prediction_method": "svm.predict() (default threshold 0.0)",
        },
    }
    ...
    print(f"    dpc_svm_no_scaler_fold_1.pkl ... dpc_svm_no_scaler_fold_{n_folds}.pkl")
    print(f"\n  Napomena: Modeli su spremljeni BEZ scalera (kao SVMlight pristup).")
    print(f"  DPC značajke su već normalizirane (frequency 0-1), što je dovoljno.")

    print("\n[DONE] DPC + SVM evaluacija zavrsena (bez StandardScaler-a).")
