...
VERZIJA S BALANSIRANIM DATASETOM I BEZ STANDARDSCALER-A (kao SVMlight).
...
# Threshold: -0.6 (umjesto defaultnog 0.0)
SVM_THRESHOLD = -0.6
...
def main():
    ...
    print("  VERZIJA S BALANSIRANIM DATASETOM I BEZ STANDARDSCALER-A")
    print("  (1:1 omjer, kao u radu; bez skaliranja, kao SVMlight)")
    print("=" * 70)
    print(f"\n  Parametri:")
    ...
    print(f"  BEZ StandardScaler-a: DPC značajke su već normalizirane (frequency 0-1).")
    ...
    fold_metrics = []
    models_dir = os.path.join(SCRIPT_DIR, "models_balanced_no_scaler")
    ...
    for fold_idx, split in enumerate(cv_splits, 1):
        ...
        # BEZ StandardScaler-a - koristimo direktno balansirane DPC značajke
        ...
        svm.fit(X_train_balanced, y_train_balanced)  # Direktno bez skaliranja

        # Predikcija na test skupu
        # Koristimo decision_function s threshold -0.6 (iz rada)
        # decision_function > -0.6 => klasa 1 (toksicni)
        # decision_function <= -0.6 => klasa 0 (netoksicni)
        ...
        model_path = os.path.join(models_dir, f"dpc_svm_balanced_no_scaler_fold_{fold_idx}.pkl")
    ...
    print(
        f"    Nasa impl. ({n_folds}-fold, balansirani, no scaler):  "
        f"Acc={summary['Accuracy']['mean']:.4f}  "
        f"MCC={summary['MCC']['mean']:.4f}"
    )

    # ------------------------------------------------------------------
    # 6. Spremanje rezultata u XLSX
    # ------------------------------------------------------------------
    results_file = os.path.join(SCRIPT_DIR, "DPC_SVM_results_balanced_no_scaler.xlsx")
    ...
    # ------------------------------------------------------------------
    # 7. Spremanje rezultata po foldovima u pickle (za evaluacijske skripte)
    # ------------------------------------------------------------------
    fold_results_file = os.path.join(SCRIPT_DIR, "dpc_svm_fold_results_balanced_no_scaler.pkl")
    ...
    fold_results_data = {
        ...
        "svm_params": {
            ...
            "threshold": SVM_THRESHOLD,  # -0.6 (iz rada, Table 1, DPC)
        },
    }
    ...
    print(f"    dpc_svm_balanced_no_scaler_fold_1.pkl ... dpc_svm_balanced_no_scaler_fold_{n_folds}.pkl")
    print(f"\n  Napomena: Modeli su trenirani na balansiranom datasetu (1:1 omjer)")
    print(f"  BEZ StandardScaler-a (kao SVMlight pristup iz originalnog rada).")

    print("\n[DONE] DPC + SVM evaluacija zavrsena (balansirani dataset, bez StandardScaler-a).")
