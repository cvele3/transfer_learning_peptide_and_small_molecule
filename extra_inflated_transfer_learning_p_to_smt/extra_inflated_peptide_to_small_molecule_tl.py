...
##############################################################
# Loading the Extra Inflated Peptide Model
PRETRAINED_MODEL_PATH = "../extra_inflated_models/extra_inflated_peptide_model.h5"
...
for split in cv_splits:
    ...
    model_save_path = f"extra_inflated_freeze_gnn_peptide_to_smile_mol_tox_fold_{fold_index}.h5"
...
for split in cv_splits:
    ...
    # Save the model for this fold with a unique filename
    model_save_path = f"extra_inflated_freeze_readout_peptide_to_smile_mol_tox_fold_{fold_index}.h5"
...
for split in cv_splits:
    ...
    # Save the model for this fold with a unique filename
    model_save_path = f"extra_inflated_freeze_all_peptide_to_smile_mol_tox_fold_{fold_index}.h5"
    model3.save(model_save_path)
    print(f"Model saved as {model_save_path}")

    
callback = EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True) # Increased patience for Method 4
...
for split in cv_splits:
    ...
    # Save the model for this fold with a unique filename
    model_save_path = f"extra_inflated_gradual_unfreezing_peptide_to_smile_mol_tox_fold_{fold_index}.h5"
