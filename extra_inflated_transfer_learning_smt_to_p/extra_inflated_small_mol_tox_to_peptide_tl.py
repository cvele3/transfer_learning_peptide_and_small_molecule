...
##############################################################
PROCESSED_DATA_FILE = "../large_layers_models/large_layers_overtrained_peptide.pkl"
...
##############################################################
# Loading the Extra Inflated Small Molecule Toxicity Model
PRETRAINED_MODEL_PATH = "../extra_inflated_models/extra_inflated_small_mol_tox_model.h5"
...
for split in cv_splits:
    ...
    model_save_path = f"extra_inflated_freeze_gnn_smile_mol_tox_to_peptide_fold_{fold_index}.h5"
...
for split in cv_splits:
    ...
    # Save the model for this fold with a unique filename
    model_save_path = f"extra_inflated_freeze_readout_smile_mol_tox_to_peptide_fold_{fold_index}.h5"
...
for split in cv_splits:
    ...
    # Save the model for this fold with a unique filename
    model_save_path = f"extra_inflated_freeze_all_smile_mol_tox_to_peptide_fold_{fold_index}.h5"
    model3.save(model_save_path)
    print(f"Model saved as {model_save_path}")

    
callback = EarlyStopping(monitor='val_loss', patience=13, restore_best_weights=True) # Increased patience for Method 4
...
for split in cv_splits:
    ...
    epochs_per_phase = 150  # Increased epochs per phase
    ...
    # Save the model for this fold with a unique filename
    model_save_path = f"extra_inflated_gradual_unfreezing_smile_mol_tox_to_peptide_fold_{fold_index}.h5"
